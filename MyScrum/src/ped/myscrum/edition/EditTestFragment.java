...
public class EditTestFragment extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        new TestInformationRetrieval(title, input, test_case, expected_result).execute("http://10.0.2.2:3000/api/owner/projects/" + project_id + "/tests/" + test_id + "/show?api_key=" + api_key);
		...
		submit.setOnClickListener(new OnClickListener() {	
			@Override
			public void onClick(View v) {
				new PostTest(title, input, test_case, expected_result, user_story, testers, state).execute("http://10.0.2.2:3000/api/owner/projects/" + project_id + "/tests/" + test_id + "/edit?api_key=" + api_key);
				getFragmentManager().popBackStackImmediate();
			}
...
private class TestInformationRetrieval extends AsyncTask<String, String, String>{
	...
	public TestInformationRetrieval(EditText title, EditText input, EditText test_case, EditText expected_result){
		this.title = title;
		this.input = input;
	...
	protected void onPostExecute(String result) {
		...
		final Map parsedData = parser.parseJson(result);
		 ...
		 title.setText((String) parsedData.get("title"));
		 input.setText((String) parsedData.get("input"));
		 test_case.setText((String) parsedData.get("test_case"));
		 expected_result.setText((String) parsedData.get("expected"));
